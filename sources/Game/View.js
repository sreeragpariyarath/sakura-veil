...
export class View
{
    ...
    setFocusPoint()
    {
        ...
        this.focusPoint = {}
        this.focusPoint.heightOffset = 1.8
        this.focusPoint.trackedPosition = new THREE.Vector3(defaultRespawn.position.x, defaultRespawn.position.y, defaultRespawn.position.z)
        this.focusPoint.isTracking = true
        this.focusPoint.position = new THREE.Vector3(defaultRespawn.position.x, defaultRespawn.position.y + 1.8, defaultRespawn.position.z)
        this.focusPoint.smoothedPosition = this.focusPoint.position.clone()
    ...
    setRoll()
    {
        ...
        this.roll.speed = 0
        this.roll.bankValue = 0
        this.roll.bankFactor = 0.6
        this.roll.damping = 4
        ...
        if(this.game.debug.active)
        {
            ...
            rollDebugPanel.addBinding(this.roll, 'bankFactor', { min: 0, max: 2, step: 0.01 })
    ...
    {
        // Forward horizontal vector on XZ plane
        const forward = new THREE.Vector3(- Math.sin(this.spherical.theta), 0, - Math.cos(this.spherical.theta)).normalize()
        // Right horizontal vector on XZ plane
        const right = new THREE.Vector3(Math.cos(this.spherical.theta), 0, - Math.sin(this.spherical.theta)).normalize()
    ...
    setMouseControls()
    {
        ...
        {
            if(this.mode === View.MODE_DEFAULT && document.pointerLockElement !== canvasElement)
            {
                if(!document.body.classList.contains('is-modal-active'))
                {
        ...
        {
            this.mouseControls.isPointerLocked = (document.pointerLockElement === canvasElement)
        })
        ...
        window.addEventListener('mousemove', (event) =>
        {
            ...
            else if(this.game.inputs.pointer.isDown && this.game.inputs.pointer.mode === Pointer.MODE_MOUSE)
            {
                movementX = this.game.inputs.pointer.delta.x
                movementY = this.game.inputs.pointer.delta.y
            }
    ...
    update()
    {
        ...
        if(this.focusPoint.isTracking)
        {
            ...
            this.focusPoint.position.y = this.focusPoint.trackedPosition.y + (this.focusPoint.heightOffset || 1.8)
        ...
        // Bank roll: simple damped follow of the ghost's current bank angle,
        // kept separate from the kick-decay spring above (that spring is only
        // stable for brief impulses, not continuous tracking)
        const bankTarget = this.game.physicalVehicle ? - this.game.physicalVehicle.bank * this.roll.bankFactor : 0
        this.roll.bankValue += (bankTarget - this.roll.bankValue) * Math.min(1, this.game.ticker.deltaScaled * 6)

        this.defaultCamera.rotation.z += this.roll.value + this.roll.bankValue
